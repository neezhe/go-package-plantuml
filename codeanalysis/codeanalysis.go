...
func StringArrayContains(intarfaceNames []string, name string) bool {
	for _, intarfaceName := range intarfaceNames {
		if strings.Contains(name, intarfaceName) {
			return true
		}
	}
	return false
...
func (this *analysisTool) typeToString(t ast.Expr, convertTypeToUnqiueType bool) (string) {
	...
	var intarfaceNames []string
	for _, m := range this.interfaceMetas {
		intarfaceNames = append(intarfaceNames, m.Name)
	}

	ident, ok := t.(*ast.Ident)
	if ok {
		if convertTypeToUnqiueType {
			if StringArrayContains(intarfaceNames, ident.Name) {
				return this.addPackagePathWhenStruct(ident.Name) + "<font color=red>========>interface</font>"
			} else {
				return this.addPackagePathWhenStruct(ident.Name)
			}
		} else {
			if StringArrayContains(intarfaceNames, ident.Name) {
				return ident.Name + "<font color=red>========>interface</font>"
			} else {
				return ident.Name
			}
		}
	...
	mapType, ok := t.(*ast.MapType)
	if ok {
		return "<font color=blue>map[" + this.typeToString(mapType.Key, convertTypeToUnqiueType) + "]" + this.typeToString(mapType.Value, convertTypeToUnqiueType) + "</font>"
	}
...
func (this *analysisTool) UML() string {
	...
	for _, structMeta1 := range this.structMetas {
		...
		uml += "\n"
		methodUml := ""
		if len(structMeta1.MethodSigns) != 0 {
			for _, methodSign := range structMeta1.MethodSigns {
				if unicode.IsUpper([]rune(methodSign)[0]) {
					methodUml += "+" + methodSign + "\n"
				} else {
					methodUml += "-" + methodSign + "\n"
				}
			}
		}
		uml = fmt.Sprintf(uml, methodUml) //把这个结构体的方法名称也塞到uml里面去。
	}
